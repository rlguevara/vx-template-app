...
pub struct CoursePerCategory {
    graphql_task: Option<GraphQLTask>,
    req_task: Option<RequestTask>,
    course_info: Option<course_by_category::ResponseData>,
}

pub enum CoursePerCategoryMessage {
    GetCourse,
    CourseAdded(Option<course_by_category::ResponseData>),
}

impl Component for CoursePerCategory{
    type Message = CoursePerCategoryMessage;
    type Properties = ();
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {
        match msg {
            Self::Message::GetCourse => {
                if let Some(graphql_task) = self.graphql_task.as_mut() {
                    let vars = course_by_category::Variables {
                        id: Some(1),
                    };
                    let task = CourseByCategory::request(graphql_task, &ctx, vars, |data| {
    ...
    fn view (&self, ctx: &Context<Self>) -> Html {
        let onclick = ctx.link().callback(|_| Self::Message::GetCourse);
        let course_information = self.course_info.clone();
        html! {
            <div>
                <button onclick={onclick}>{"Get Course"}</button>
                <div>
                    <p>{format!("{:?}", course_information)}</p>
                </div>
            </div>
        }
    }
