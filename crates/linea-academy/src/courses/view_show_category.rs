...
use log::*;
use yew::prelude::*;
use crate::components::categories::Categories;
...
pub struct ShowCategory {
    ...
    category: Option<category_data::ResponseData>,
}

#[derive(Debug)]
pub enum ShowCategoryMessage {
    GetCategory,
    CategoryReceived(Option<category_data::ResponseData>),
}

impl Component for ShowCategory {
    type Message = ShowCategoryMessage;
    type Properties = ();

    fn create(ctx: &Context<Self>) -> Self {
        let show_data = ctx.link().send_message(Self::Message::GetCategory);
        Self {
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {
        match msg {
            Self::Message::GetCategory => {
                if let Some(graphql_task) = self.graphql_task.as_mut() {
                    let vars = category_data::Variables {
                    };
                    let task = CategoryData::request(graphql_task, &ctx, vars, |data| {
